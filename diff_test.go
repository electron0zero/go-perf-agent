...
+z
`
	got := funcsFromPatchHeaders(patch)
	if len(got) != 2 {
...
func TestFuncsForRanges(t *testing.T) {
	...
	if got := funcsForRanges(path, [][2]int{{4, 4}}); len(got) != 1 || got[0].Func != "Foo" || got[0].Lines != "3-5" {
		t.Errorf("range in Foo gave %+v", got)
	}
	if got := funcsForRanges(path, [][2]int{{8, 8}}); len(got) != 1 || got[0].Func != "(*Bar).Baz" {
		t.Errorf("range in Baz gave %+v", got)
	}
	if got := funcsForRanges(path, [][2]int{{1, 1}}); len(got) != 0 {
...
func TestSymbolFor(t *testing.T) {
	...
	for _, c := range cases {
		if got := symbolFor(c.pkgDir, c.recv, c.name); got != c.want {
