...
`
	if d, p := parseBenchstat(csv, "sec/op"); d != "-10.00%" || p != "0.002" {
		t.Errorf("sec/op = (%q,%q), want (-10.00%%,0.002)", d, p)
	}
	if d, p := parseBenchstat(csv, "allocs/op"); d != "-50.00%" || p != "0.001" {
		t.Errorf("allocs/op = (%q,%q), want (-50.00%%,0.001)", d, p)
	}
	if d, p := parseBenchstat(csv, "B/op"); d != "" || p != "" {
...
`
	if _, p := parseBenchstat(csv, "sec/op"); p != "0.002" {
