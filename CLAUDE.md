...
- The model is the core driver: it reasons about the code, forms hypotheses, authors benchmarks, and applies changes. benchstat objectively verifies each keep/reject, so a win is proven by measurement, not asserted - the gate guards the model, it does not replace it.
